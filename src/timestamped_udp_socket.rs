...
use crate::raw_udp_socket::TimestampingMode;
...
impl TimestampedUdpSocket {
    ...
    pub async fn recv(&self, clock: &impl Clock, buf: &mut [u8]) -> std::io::Result<RecvResult> {
    ...
    async fn fetch_send_timestamp(
        ...
        if let Ok(send_timestamp) = tokio::time::timeout(timeout, fetch).await {
            ...
            tracing::info!("Packet without timestamp (waiting for timestamp timed out)");
            Ok(None)
        }
    }
    ...
    async fn fetch_send_timestamp_help(
        ...
        tracing::trace!(
            "waiting for timestamp socket to become readable to fetch a send timestamp"
        );
        ...
        loop {
            ...
            match result {
                ...
                Err(e) => {
                    tracing::info!("Error fetching timestamp: {e:?}");
                    return Err(e);
                }
            }
        }
        ...
        match msg {
            ...
            ControlMessage::ReceiveError(error) => {
                ...
                if error.ee_errno as libc::c_int != libc::ENOMSG {
                    tracing::info!( "error message on the MSG_ERRQUEUE: expected message {}, it has error code {}", expected_counter, error.ee_data,);
                }
...
mod tests {
    ...
    async fn timestamping_reasonable(p1: u16, p2: u16) {
        ...
        let clock = clock_steering::unix::UnixClock::CLOCK_REALTIME;
